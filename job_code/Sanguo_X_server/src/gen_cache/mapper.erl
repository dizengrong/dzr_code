...
%%
make_delete(Map, RecordData) ->
	case Map#map.key_classic of
		0 ->
			Fields = lists:sublist(Map#map.fields, length(Map#map.fields) - length(Map#map.ignored_fields)),
			[_ | FieldValues] = erlang:tuple_to_list(RecordData),
			Values = lists:sublist(FieldValues, length(FieldValues) - length(Map#map.ignored_fields)),
			[_ | FieldSpecs] = erlang:tuple_to_list(Map#map.fields_spec),
			FieldSpecs1 = lists:sublist(FieldSpecs, length(FieldSpecs) - length(Map#map.ignored_fields)),
		...
		_ ->
			...
			[_ , KeySpec | _FieldSpecs] = erlang:tuple_to_list(Map#map.fields_spec),
			KeySpec1 = make_key_spec(KeySpec),
			WhereCond = make_where(Map#map.key_fields, KeyValues, KeySpec1)
	end,
	lists:concat(["delete from ", Map#map.sql_tab, " where ", WhereCond]).

make_select(Map, KeyValue, IsSelectFromAll) ->
	case Map#map.key_classic of
		0 ->
			Fields1 = make_fields(Map#map.fields, "", Map#map.ignored_fields);
		_ ->
	...
	case IsSelectFromAll of
		...
		false -> 
			[_ , KeySpec | _FieldSpecs] = erlang:tuple_to_list(Map#map.fields_spec),
			KeySpec1 = make_key_spec(KeySpec),
			WhereCond = make_where(Map#map.key_fields, [KeyValue], KeySpec1)
	end,
	lists:concat(["select ", Fields1, " from ", Map#map.sql_tab, " where ", WhereCond]).

make_fields(Map) ->
	case Map#map.key_classic of
		0 -> Rest = Map#map.fields;
		_ -> [_Field | Rest] = Map#map.fields		
	end,
	S = make_key_fields(Map#map.key_classic, Map#map.key_fields),
	make_fields(Rest, S, Map#map.ignored_fields).
...
make_insert(Map, RecordData) ->
	...
	lists:concat(["INSERT INTO ", Map#map.sql_tab, "(", Fields, ") VALUES (", Values, ")"]).
	
make_values(Map, RecordData) ->
	[_, Key | FieldValues] = erlang:tuple_to_list(RecordData),
	[_, KeySpec | FieldSpecs] = erlang:tuple_to_list(Map#map.fields_spec),
	[_KeyField | FieldList] = Map#map.fields,
	S1 = make_key_values(Map, Key, KeySpec),
	S2 = make_field_values(FieldValues, FieldList, FieldSpecs, Map#map.ignored_fields),
	case S2 of
		[] -> S1;
		_ -> S1 ++ ", " ++ S2
	end.
...
make_update(Map, RecordData) ->
	...
	KeySpec1 = make_key_spec(KeySpec),
	WhereCond = make_where(Map#map.key_fields, KeyValues, KeySpec1),
	[_Field | RestFields] = Map#map.fields,
	Set = make_set(RestFields, FieldValues, FieldSpecs, Map#map.ignored_fields, []),
	lists:concat(["UPDATE ", Map#map.sql_tab, " SET ", Set, " WHERE ", WhereCond]).
