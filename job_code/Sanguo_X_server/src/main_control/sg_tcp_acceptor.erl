...
init({LSock}) ->
	...
	?INFO(tcp_acceptor, "accept socket ~w",[LSock]),
    ...
    case prim_inet:getopts(LSock, [active, nodelay, keepalive, delay_send, priority, tos]) of
        {ok, Opts} ->
            case prim_inet:setopts(Sock, Opts) of
                ok    -> 
					prim_inet:setopts(Sock, [{nodelay, true}, {delay_send, false}]),
%% 					prim_inet:setopts(Sock, [{high_watermark, 131072}]),
%% 					prim_inet:setopts(Sock, [{low_watermark, 65536}]),
					ok;
                Error ->
					?ERR(acceptor,"set acceptor opt failure ~w",[Error]),
                    gen_tcp:close(Sock),
                    Error
            end;
        Error ->
...
%% 开启客户端服务
start_client(Sock) ->
    {ok, Child} = supervisor:start_child(sg_tcp_client_sup, []),
