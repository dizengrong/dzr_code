...
%%          {stop, Reason, NewStateData}
%% --------------------------------------------------------------------
handle_event(_Event, StateName, BattleData) ->
...
get_mer_info(Camp, ID, MerList, MakeTeam, Array) ->
	...
	ArrayFun = 
		fun (Role, Arr) ->
			...
			array:set(Pos, role_2_bs(Role), Arr)
		end,
...
role_2_bs(Role) ->
	?INFO(battle, "rank = ~w", [Role#role.gd_roleRank]),
...
handle_command_ex(BattleData, Repeat) ->
	...
	NBattleData = 
		...
		true ->
			...
			set_battle_status(Src, NSrcStat#battle_status {cmd = ?UNDEFINED}, BattleData1)
		end,
	...
	NBattleData2 = 
		...
		true ->
			NBattleData1#battle_data {
				%% update the order list according to the speed
				attorder = update_order_list(tl(OrderList), NBattleData1)
			}
		end,
			...
			false ->
				case lists:keysearch(?BUFF_COUNTER, #buff.name, Buffs) of
					{value, #buff{value = Counter}} ->
						Counter;
					false ->
						case random:uniform() =< TarStat#battle_status.counter / 1666 of
							true -> 100;
							false -> 0
						end
				end
...
pre_attack(Src, Tar, _AttSpec, BattleData) ->
	...
	IsHit = Hit * 10 / (Hit * 11 + Dodge) + Param6 * (AStar - DStar),
	?INFO(battle, "Hit = ~w, Dodge = ~w, Param = ~w, IsHit = ~w", [Hit, Dodge, Param6, IsHit]),
	
	NIsHit = max(0.4, min(0.94, IsHit)),
	
	?INFO(battle, "Src = ~w, NIsHit = ~w", [Src, NIsHit]),
	random:uniform() =< NIsHit.
...
do_attack(Src, Tar, AttSpec, BattleData) ->
	...
	{ALevel, DLevel} = {SrcStat#battle_status.star, NTarStat#battle_status.star},
	{Crit, Luck} = {
	...
	IsCrit = random:uniform() =< (1 - Luck * 12 / (Luck * 12 + Crit * 5)), 
	Deadly = 
		case lists:keysearch(?BUFF_FATAL, #buff.name, 
			SrcStat#battle_status.buff) of
			false -> 0;
			{value, #buff {value = V}} -> V
		end,	
	
	?INFO(battle, "Src = ~w, Job = ~w, Att = ~w, Def = ~w, ALevel = ~w, DLevel = ~w, Crit = ~w, Luck = ~w, Dead = ~w",
		[Src, Job, Att, Def, ALevel, DLevel, Crit, Luck, Deadly]),
	...
	Damage0 = 
		Att * (1 - Def / (Def + Param1)) * (1 + Param2 * (ALevel - DLevel)) * 
	...
	true ->
		case pre_attack(Src, Tar, AttSpec, BattleData) of
			false ->
				...
				?INFO(battle, "Src = ~w, Tar = ~w, Result: miss", [Src, Tar]),
...
handle_attack_info(SkillId, Src, AttInfoList, [AttInfo | Rest], BattleData) ->
	...
	case AttInfo#attack_info.is_miss    == true  orelse 
		...
		false ->
			...
			NSDamDeal = SDamDeal + max(0, -AttInfo#attack_info.hp_inc),
			NSDamSuff = SDamSuff + max(0, -Counter) + max(0, -Rebound),
			NTDamDeal = TDamDeal + max(0, -Counter) + max(0, -Counter),
			NTDamSuff = TDamSuff + max(0, -AttInfo#attack_info.hp_inc),
...
assist(SkillId, Src, [AssSpec | Rest], AttInfoList, BattleData) ->
	...
	NAttInfo =
		case IsMiss of
			true -> AttInfo;
			false -> assist_1(Tar, Eff, AttInfo, BattleData)
		end,
	...
	NAttInfo = 
		case Type of
			heal ->
				...
				AttInfo#attack_info {hp_inc = Inc};
			mana -> 
				Inc = 
					if (ByRate == true) ->
			...
			{BuffInfo, NState} = 
				case BuffName of
					?BUFF_TOXIC ->
						...
						NewHp  = max(0, State#battle_status.hp - HpLost),
						BInfo  = 
							#buff_info {
								name      = Buff#buff.name,
								owner     = Pos,			
...
update_buffs(Pos, [{Buff, Rate, Op} | Rest], BuffInfoList, BattleData) ->
	...
	case random:uniform() > Rate of
		...
		false ->
			State  = get_battle_status(Pos, BattleData),
			ByRate = Buff#buff.by_rate,
			Value  = if (ByRate == true) -> round(Buff#buff.value); true -> round(Buff#buff.value * 100) end,
			case Op of
				add ->
					BuffInfo = 
						#buff_info {
...
get_hp_lose_value(Hp, #buff {name = ?BUFF_TOXIC, by_rate = ByRate, value = Value}) ->
	if (ByRate == true) ->
		Value;
	true ->
		round(Hp * Value)
...
get_damage_value(Dm, Pos, BattleData) ->
	...
	if (F =/= 0) ->
		?INFO(battle, "Damage is reduced ~w% by scorn buff. ", [F * 100]);
	true ->
		ok
	end,
	round(Dm * (1 - F)).
...
get_battle_award(BattleData) ->
	...
	true ->
		...
		DispatchList = get_items_dispatch(IDList, Items),
		
		?INFO(battle, "Items = ~w, DispatchList = ~w", [Items, DispatchList]),
		#battle_award {
			gold   = 0,
			silver = 0,
			donate = 0,
...
get_items_dispatch(Ids, ItemInfo) ->
	get_items_dispatch(Ids, ItemInfo, []).
	
get_items_dispatch([], _, DspList) -> DspList;
get_items_dispatch([Id | Rest], ItemInfo, DspList) ->
	Items = get_items_dispatch_1(ItemInfo),
	get_items_dispatch(Rest, ItemInfo, [{Id, Items} | DspList]).
