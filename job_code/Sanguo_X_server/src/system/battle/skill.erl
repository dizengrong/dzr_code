...
%% 威慑4方: 进行N次物理攻击, 并且随机嘲讽N一个目标, 在嘲讽状态下受到攻击不加怒气
%% N = max(3, Level + 2 / 3)
handle_skill(SkillId = 104, Src, Tar, Level, Param, BattleData) ->	
	...
	Debuff    = #buff{name = ?BUFF_SCORNED, duration = 1, settle = pre, value = Src},
...
handle_skill(SkillId = 109, Src, Tar, Level, Param, BattleData) ->		
	...
	{_, NBattleData} = lists:foldl(F, {true, BattleData}, lists:seq(1, max(4, 1 + (Level + 2 / 3)))),
	%% no buff to add
...
handle_skill(SkillId = 112, Src, Tar, _Level, Param, BattleData) ->
	AttSpec = #attack_spec {
		addition = ?p1,				
		targets = [Tar]		
	},
