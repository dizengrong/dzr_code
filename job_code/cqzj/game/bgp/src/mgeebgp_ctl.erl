...
-spec init() -> 'ok'.
init() ->
    ets:new(mgeebgp_ctl_cmds, [named_table, set, public]),
    ets:new(mgeebgp_ctl_host_cmds, [named_table, set, public]),
    ok.
...
process(["status"]) ->
    ...
    case lists:keysearch(mgeel, 1, application:which_applications()) of
        false ->
...
print_usage() ->
    ...
    ?PRINT(
      "Usage: mgeebgpctl [--node nodename] command [options]~n"
      "~n"
      "Available commands in this node node:~n"
      ++ FmtCmdDescs ++
      "~n"
      "Examples:~n"
      "  mgeebgpctl restart~n"
      "  mgeebgpctl --node node@host restart~n"
      "  mgeebgpctl vhost www.example.org ...~n",
     []).
