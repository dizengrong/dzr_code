% %% 多人副本(非组队副本，以后有开发组队副本的话应该叫mod_team_fb)
% %% 这种副本是指可以多人进入的，非组队要求的
-module (mod_multi_player_fb).
...
% handle({init_fb_map_info, RoleID, EnterMapId, EnterPos, FbType, FbId}) ->
% 	erlang:put(enter_pos, {EnterMapId, EnterPos}),
% 	erlang:put(fisrt_enter_this_fb, true),
% 	erlang:put(in_single_fb_flag, true),
...
% loop(_MapId, _NowSec) ->
% 	RoleID = erlang:get(fb_owner),
% 	case erlang:get(ready_to_close) of
% 		undefined -> ignore;
% 		LeftSeconds -> do_ready_close_broadcast(RoleID, LeftSeconds)
% 	end,
% 	ok.

% do_ready_close_broadcast(RoleID, LeftSeconds) when LeftSeconds =< 0 -> do_close_fb(RoleID);
% do_ready_close_broadcast(RoleID, LeftSeconds) ->
% 	if
...
% is_in_this_fb() -> 
% 	case erlang:get(in_single_fb_flag) of
% 		true -> true;
% 		_ -> false
% 	end.
...
% 	lists:concat(["single_fb_map_", MapId, "_", RoleID]).
...
% 			case SingleFBBaseinfo#single_fb_baseinfo.entry_npc of
% 				0 -> true;
% 				NPCId  -> %% todo: 检测玩家与npc的距离 
% 					true;
% 				_ -> {error, <<"请求副本信息时的npc参数错误">>}
% 			end;
% 		_ ->
% 			{error, <<"您的等级不够，副本暂未开启">>}
% 	end.
...
% 			mod_map_copy:create_copy(MapId, MapPID),
% 			EnterPos         = mod_map_actor:get_actor_pos(RoleID, role),
% 			EnterMapId       = mgeem_map:get_mapid(),
% 			global:send(MapPID, {?MODULE, {init_fb_map_info, RoleID, EnterMapId, EnterPos, FbType, FbId}}),
% 			%% 传送到新地图
...
% send_role_quit(RoleID) ->
% 	{EnterMapId, EnterPos} = erlang:get(enter_pos),
...
% 			end,
% 			case erlang:get(fisrt_enter_this_fb) of
% 				true -> %% 第一次进入
% 					erlang:put(fisrt_enter_this_fb, false),
