...
-spec init() -> 'ok'.
init() ->
    ets:new(mgeerec_ctl_cmds, [named_table, set, public]),
    ets:new(mgeerec_ctl_host_cmds, [named_table, set, public]),
    ok.
...
process(["status"]) ->
    ...
    case lists:keysearch(mgeerec, 1, application:which_applications()) of
        false ->
