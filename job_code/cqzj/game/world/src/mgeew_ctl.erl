...
-spec init() -> 'ok'.
init() ->
    ets:new(mgeew_ctl_cmds, [named_table, set, public]),
    ets:new(mgeew_ctl_host_cmds, [named_table, set, public]),
    ok.
...
process(["status"]) ->
    ...
    case lists:keysearch(mgeew, 1, application:which_applications()) of
        false ->
