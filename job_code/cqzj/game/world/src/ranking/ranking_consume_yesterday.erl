...
-module(ranking_consume_yesterday).
...
init2(RankInfo) ->
    init3(RankInfo,?MODULE,?DB_ROLE_CONSUME_YESTERDAY_RANK_P,fun(E1,E2) -> cmp(E1,E2) end).
...
get_rank_data()->
    db:dirty_match_object(?DB_ROLE_CONSUME_YESTERDAY_RANK_P,#p_role_consume_today_rank{_ = '_'}).

rank() ->
    rank2(?MODULE,?DB_ROLE_CONSUME_YESTERDAY_RANK_P,fun(E1,E2) -> cmp(E1,E2) end).
...
update({RoleID,Score,Now}) ->
	...
	ranking_minheap:update_heap(RoleConsumeTodayRank,RoleID,?DB_ROLE_CONSUME_YESTERDAY_RANK_P,?MODULE,?MODULE);
update(_) ->
	ignore.
...
%%重置今日排行榜数据
reset_consume_rank() ->
	{RankInfo,_} = get({rank_info,?CONSUME_YESTERDAY_RANK_ID}),
	case get_rank_data() of
		[] ->
			ignore;
		_ ->
			mnesia:clear_table(?DB_ROLE_CONSUME_YESTERDAY_RANK_P)
	end,
	init2(RankInfo).
	...
	case db:dirty_match_object(?DB_ROLE_ACCGOLD,#r_role_accgold{_ = '_'}) of
		...
		List -> 
			lists:foreach(fun(#r_role_accgold{role_id=RoleID,consume_list=ConsumeList1,last_consume_time=LastConsumeTime})->
								  case lists:keyfind(Yesterday, 1, ConsumeList1) of
									  false->
										  ignore;
									  {Yesterday,UseGold}->
										  Score = calc_total_score(UseGold),
										  LimitScore = cfg_rank_score:socre_limit(?CONSUME_YESTERDAY_RANK_ID),
...
send_reward_letter() ->
	...
	mod_rankreward_server:send_consume_reward_letter(?CONSUME_YESTERDAY_RANK_ID,RoleRankList).
